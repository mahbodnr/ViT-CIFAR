    ...
    def step(self, closure=None):
        for group in self.param_groups:
            for p in group['params']:
                ...
                # update pre care
                if group.get('nnmf', False):
                    p.grad /= group["contributions"] # = batch * output_size 1 * output_size 2
                ...
                if group.get('nnmf', False):
                    p.data.mul_(0.5 * torch.tanh(-step_size * (exp_avg / denom)) + 1.0)
                else:
                    p.data.addcdiv_(-step_size, exp_avg, denom)
                # update post care
                if group.get('nnmf', False):
                    p.data /= p.data.sum(dim=0, keepdim=True) # normalize
                    p.data.clamp_(min=float(group["weight_threshold"]), max=None) # clamp
                    p.data /= p.data.sum(dim=0, keepdim=True) # normalize
