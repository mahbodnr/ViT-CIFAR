...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_map = F.softmax(torch.einsum("bhif, bhjf->bhij", Q, K)/self.sqrt_d, dim=-1) #(b,h,n,n)
...
class AFTFull(nn.Module):
    def __init__(self, features:int, seq_len:int, head:int=1, dropout:float=0.):
        super().__init__()
        '''
    ...
    def forward(self, x):
        B, T, _ = x.shape
        Q = self.Wq(x)#.view(B, T, self.features)
        K = self.Wk(x)#.view(B, T, self.features)
        V = self.Wv(x)#.view(B, T, self.features)
        Q_sig = torch.sigmoid(Q)
        weighted = torch.exp(self.w) @ torch.mul(torch.exp(K), V) / (torch.exp(self.w) @ torch.exp(K))
...
class AttentionFreeTransformerEncoder(TransformerEncoder):
    def __init__(self, mode:str, features:int, seq_len:int, mlp_hidden:int, head:int=1, dropout:float=0.):
        super(AttentionFreeTransformerEncoder, self).__init__(features, mlp_hidden, head, dropout)
        if mode=="full":
            self.attention = AFTFull(features, seq_len, head=head, dropout=dropout)
        elif mode=="local":
            raise NotImplementedError
        elif mode=="conv":
            raise NotImplementedError
...
if __name__=="__main__":
    ...
    draw_graph(MHA, graph_name='Multy Head Attention', input_size=input_size, expand_nested=True, save_graph=True, directory="imgs")   
    net = TransformerEncoder(features=input_size[-1], mlp_hidden=256, head=8)
    draw_graph(net, graph_name='Transformer Encoder', input_size=input_size, expand_nested=True, save_graph=True, directory="imgs")   
    AFT = AttentionFreeTransformerEncoder(mode="full", features=input_size[-1], seq_len=input_size[1], mlp_hidden=256, head=1)
    draw_graph(AFT, graph_name='Attention Free Transformer Encoder', input_size=input_size, expand_nested=True, save_graph=True, directory="imgs")